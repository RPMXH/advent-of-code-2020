...
def find_all_containers(rule_map, containee):
    possible_containers = []
    for rule_key, rule_values in rule_map.items():
        for entry_value in rule_values:
            if containee in entry_value:
                possible_containers.append(rule_key)

    additional_containers = []
    if len(possible_containers) > 0:
        for possible_container in possible_containers:
            additional_containers.extend(find_all_containers(rule_map, possible_container))
...
print(len(find_all_containers(rule_map, "shiny gold")))
